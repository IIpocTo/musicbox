	...
	try:
		...
		for j, album in enumerate(albums_by_artist):
			try:
				...
				print('...Album: {}/{} from artist {}'.format(j+1, len(albums_by_artist), el['name']))
...
if __name__ == '__main__':
	...
	try:
		...
		for artist in res_artists:
			...
			got_artist = artists.find({ '_id': artist['_id']}).limit(1)
			if got_artist.count() != 0:
				artists.update_one({ '_id': artist['_id']}, { '$set': artist_without_id })
			else:
				artists.insert_one(artist)
		for album in res_albums:
			album_without_id = dict(album)
			del album_without_id['_id']
			got_album = albums.find({ '_id': album['_id']}).limit(1)
			if got_album.count() != 0:
				albums.update_one({ '_id': album['_id']}, { '$set': album_without_id })
			else:
				albums.insert_one(album)
		for track in res_tracks:
			track_without_id = dict(track)
			del track_without_id['_id']   
			got_track = tracks.find({ '_id': track['_id']}).limit(1)
			if got_track.count() != 0:
				tracks.update_one({ '_id': track['_id']}, { '$set': track_without_id })
			else:
